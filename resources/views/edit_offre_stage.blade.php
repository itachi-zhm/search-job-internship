<form action="{{ route('stage.update', ['id_offre' => $offre->id_offre]) }}" method="POST">
    @csrf
    @method('PUT')
    ...
    <input type="number" name="remuneration_stage" value="{{ $stage->remuneration_stage }}">
    
    <input type="text" name="duree_stage" value="{{ $stage->duree_stage }}">
